window.addEventListener('load', () => {
  ...
  const store = new Vuex.Store({
    state() {
      return {
        ...
        renderingTable: null,
        stickyScrollReRender: true,
      };
    },
    mutations: {
      setStickyScrollReRender(state, value) {
        state.stickyScrollReRender = value;
      },
      ...
      renderTable({ state, commit, getters }, href) {
        (async () => {
          ...
          if (!response.ok) {
            ...
            commit('setStickyScrollReRender', !state.stickyScrollReRender);
          }
        })();
      },
      seturl({ getters }) {
  ...
  Vue.component('stickyScroll', {
    ...
    props: ['reInitWatcher'],
    template: `
      <div :class="{'twpx-sticky-scroll': true, 'twpx-sticky-scroll--scrollable': supportsTouch()}">
    ...
    methods: {
      ...
      thumbMousedown(e) {
        ...
        document.body.classList.add('twpx-no-select');
      },
      thumbClick(e) {
        e.stopPropagation();
      },
      ...
      async initContentWidth() {
        do {
          const isEmpty = 
            !this.$refs.content
            || (this.$refs.content.querySelector('.vue-tf-table-wrapper') && !this.$refs.content.querySelector('table.table'))
    ...
    mounted() {
      ...
      document.addEventListener('mouseup', () => {
        if (this.thumbClickedFlag) {
          ...
          document.body.classList.remove('twpx-no-select');
        }
      });
  ...
  const app = new Vue({
    ...
    template: `
      <div class="b-registry-report">
        ...
        <hr>
        <sticky-scroll :reInitWatcher="$store.state.stickyScrollReRender">
